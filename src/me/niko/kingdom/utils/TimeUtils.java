...
public class TimeUtils {
	...
	private static final ThreadLocal<StringBuilder> mmssBuilder = ThreadLocal
			.withInitial((Supplier<? extends StringBuilder>) StringBuilder::new);
	private static final SimpleDateFormat dateFormat = new SimpleDateFormat("MM/dd/yyyy HH:mm");

	private TimeUtils() {
	}

	public static String formatFancy(float value) {
		if (value >= 60.0f) {
            return TimeUtils.formatIntoMMSS((int) value);
        } else {
