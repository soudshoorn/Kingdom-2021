...
public final class LunarClientAPI implements Listener {
    ...
    public LunarClientAPI(JavaPlugin plugin) {
        ...
        this.plugin.getServer().getPluginManager().registerEvents(new LunarClientLoginListener(this), this.plugin);
	}
    ...
    private void registerPluginChannel(String bukkitChannel) {
        Messenger messenger = getPlugin().getServer().getMessenger();
        messenger.registerOutgoingPluginChannel(getPlugin(), bukkitChannel);
        messenger.registerIncomingPluginChannel(getPlugin(), bukkitChannel, (channel, player, bytes) -> {
    ...
    public void unregisterPlayer(Player player, boolean quit) {
        ...
        if (quit) {
            ...
            playersNotRegistered.add(player.getUniqueId());
            this.plugin.getServer().getPluginManager().callEvent(new LCPlayerUnregisterEvent(player));
        }
    }
    ...
    public boolean sendPacket(Player player, LCPacket packet) {
        ...
        if (isRunningLunarClient(playerId)) {
            ...
            player.sendPluginMessage(getPlugin(), channel, LCPacket.getPacketData(packet));
